<template>
  <div style="height: 100%">
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <i class="el-icon-edit"></i>&nbsp;&nbsp;
        <span>AiBlog分类列表</span>
      </div>
      <div class="text item">
        <el-button icon="el-icon-back" @click="$router.go(-1)"></el-button>
        <el-button style="float: right" type="primary" @click="addType">新建分类</el-button>
        <el-input v-if="inputTypeVisible" v-model="inputTypeValue" ref="saveTypeInput"
                  style="width: 150px;float: right" @keyup.enter.native="inputType" @blur="inputType">
        </el-input>
        <el-table :data="typeList" border style="margin-top: 5px;width: 100%" :height="tableHeight">
          <el-table-column width="50" align="center">
            <template slot-scope="scope">
              {{ (scope.$index + 1) + pageSize * (pageNum - 1) }}
            </template>
          </el-table-column>
          <el-table-column align="center" prop="name" label="名字"></el-table-column>
          <el-table-column align="center" prop="count" label="博客数量"></el-table-column>
          <el-table-column align="center" label="操作">
            <template slot-scope="scope">
              <el-button @click="showBlog(scope.row)">内容</el-button>
              <el-tooltip effect="dark" content="博客数量为0时可删除" placement="bottom">
                <el-popconfirm icon="el-icon-info" icon-color="red"
                               title="这个分类确定删除吗？" @onConfirm="deleteType(scope.row)">
                  <el-button slot="reference" type="danger" plain>删除</el-button>
                </el-popconfirm>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination v-if="show" style="text-align: center;padding-top: 15px" :current-page="currentpage"
                       background :page-count="page" @current-change="handleCurrentChange"
                       layout="prev, pager, next">
        </el-pagination>
      </div>
    </el-card>
    <el-dialog :title="title" align="center" :modal-append-to-body="false" :visible.sync="dialogVisible" width="50%">
      <el-table :data="blogList" stripe height="300">
        <el-table-column prop="title" label="标题" align="center"></el-table-column>
        <el-table-column prop="createTime" label="创建时间" align="center"></el-table-column>
      </el-table>
    </el-dialog>
  </div>
</template>
<script>
import {
  addType,
  deleteBlog,
  deleteType,
  getBlogById,
  getBlogByTypeId,
  getBlogList,
  getType,
  updatePublish
} from "../api";

export default {
  name: "notes",
  data() {
    return {
      tableHeight: document.documentElement.clientHeight - 300,
      multipleSelection: [],
      input: '',
      typeList: [],
      pageNum: 1,
      pageSize: 7,
      page: 1,
      currentpage: 1,
      show: true, // 分页展示
      title: '', // 弹出框标题
      blogList: [], // 弹出框内容
      dialogVisible: false, // 预览展示
      inputTypeVisible: false,
      inputTypeValue: ''
    };
  },
  created() {
    this.getType(this.pageNum, this.pageSize);
  },
  methods: {
    getType(pageNum, pageSize) { // 获取分类列表
      getType({
        pageNum: pageNum,
        pageSize: pageSize
      })
        .then(res => {
          this.typeList = res.typeList
          this.page = res.pages;
          this.pageNum = res.pageNum;
        })
    },
    inputType() {
      let inputTypeValue = this.inputTypeValue;
      if (inputTypeValue.trim()) {
        let param = new URLSearchParams();
        param.append("name", inputTypeValue)
        addType(param)
          .then(res => {
            if (res.status == 1) {
              this.$notify({
                type: "success",
                title: res.msg
              })
              this.getType(this.pageNum,this.pageSize);
            } else {
              this.$notify({
                type: "error",
                title: res.msg
              })
            }
          })
      }
      this.inputTypeVisible = false;
      this.inputTypeValue = '';
    }
    ,
    addType() {
      this.inputTypeVisible = true;
      this.$nextTick(_ => {
        this.$refs.saveTypeInput.$refs.input.focus();
      });
    },
    deleteType(row) {
      if (row.count != 0) {
        this.$message.error('此分类中含有博客，无法删除');
        return
      }
      deleteType({id: row.id})
        .then(res => {
          if (res.status == 1) {
            this.$notify({
              type: "success",
              title: res.msg
            })
            if (this.typeList.length == 1) {
              this.getType(this.pageNum - 1, this.pageSize);
            } else {
              this.getType(this.pageNum, this.pageSize);
            }
          } else {
            this.$notify({
              type: "error",
              title: res.msg
            })
          }
        })
    },
    showBlog(row) { // 弹出框
      this.title = row.name
      getBlogByTypeId({typeId: row.id, pageSize: 10000})
        .then(res => {
          this.blogList = res.blogList;
          this.dialogVisible = true;
        })
    },
    handleCurrentChange(val) {
      this.getType(val, this.pageSize);
    },
  }
};
</script>
<style scoped lang="scss">

</style>
